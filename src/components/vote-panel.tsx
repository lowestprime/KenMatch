...
export function VotePanel({
  ...
  return (
    <div className="panel space-y-5">
      <div>
        <div className="eyebrow">Priority credits</div>
        <h2 className="mt-2 font-display text-2xl font-semibold text-foreground">Spend limited voice on the Kens you want to move up</h2>
        <p className="mt-2 text-sm leading-7 text-muted">
          Stronger support costs disproportionately more than broad support. That lets people show intensity without turning the board into a money contest or letting one person dominate the feed.
        </p>
      </div>
      <form action={formAction} className="space-y-4">
        ...
        <div className="metric-grid">
          ...
          <div className="metric-card"><div className="eyebrow">Total voice</div><div className="metric-value">{totalCredits}</div></div>
        </div>
        ...
        <label className="space-y-2 text-xs uppercase tracking-[0.22em] text-muted">
          Why this Ken deserves more priority
          <textarea
            name="rationale"
            value={rationale}
            ...
            rows={4}
            maxLength={280}
            placeholder="Explain the public value, timing, or evidence quality in plain language."
            className="field"
            disabled={disabled || isPending}
          />
        </label>
