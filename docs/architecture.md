...
- `middleware.ts` enforces host filtering and cross-site request blocking for public deployments.
- `Dockerfile` runs the standalone server generated by `npm run build` as a non-root user.
- `docker-compose.synology.yml` mounts persistent app data, uses a read-only root filesystem, and binds only to loopback.
...
- `src/lib/seed-plus.ts` adds attestation state, timing, audit updates, structured treasury entries, and sponsor commitments.
