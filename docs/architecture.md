...
- `src/app/about/page.tsx`
  - Public About / Contact page with owner-only online editing, external
    community boundaries, and the integrated changelog.
- `src/app/faq/page.tsx`, `src/components/faq-explorer.tsx`, and `src/lib/faq.ts`
  - Searchable, deep-linkable product and governance answers, visible source
    citations, accurate FAQ JSON-LD, the product-truth matrix, and the contact
    route.
- `src/app/glossary/page.tsx`, `src/components/glossary-explorer.tsx`, and
  `src/lib/glossary.ts`
  - Searchable operational terms with implementation states and governing
    routes.
...
- Production cookies remain secure except inside the exact non-live,
  token-bound `kenmatch-audit-app` snapshot-lab origin. The isolated exception
  exists only so production-mode Docker evidence can exercise contributor,
  moderator, administrator, and owner role boundaries over its internal HTTP
  network.
...
- `src/lib/seo.ts`, `src/app/robots.ts`, and `src/app/sitemap.ts` own canonical
  metadata, public/private indexing policy, and public-only static/dynamic
  discovery. Valid category and lane pages are indexable; search, sort, stage,
  pagination, combined-filter, private, and API surfaces are noindex.
- `scripts/audit-seo.mjs` verifies actual candidate responses for titles,
  descriptions, canonicals, social records, schemas, headings, sitemap
  boundaries, query variants, private headers, and structured 404 behavior.
  The policy, evidence, and current-intent research are in
  `docs/seo-implementation.md` and `docs/marketing/seo-and-content-strategy.md`.
...
- `visual-audit/`, `docker-compose.visual-audit-lab.yml`, and
  `docker-compose.visual-audit-live.yml` implement the private deterministic
  release archive. The protected inventory endpoint is dual-gated by an
  owner/admin session and independent audit token; visitor telemetry is
